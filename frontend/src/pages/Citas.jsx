...
const Citas = () => {
  ...
  const confirmarYRecepcionar = () => {
    // Navegamos a la nueva pantalla que vas a crear
    ...
    <Layout tituloNavbar="Agenda de Citas">
      <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-gray-50">
              {!loading && citas.map((cita) => (
                <tr key={cita.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-4 font-bold text-gray-700">🕒 {cita.hora_inicio} - {cita.hora_fin}</td>
                  <td className="px-4 py-4">
                    <div className="text-sm font-bold text-gray-800">{cita.vehiculoPlaca}</div>
                    <div className="text-[11px] text-gray-400 uppercase">{cita.vehiculo?.modelo} | {cita.vehiculo?.cliente?.apellidos}</div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex flex-col">
                      <span className="text-[12px] font-bold text-gray-800">{cita.servicio?.nombre}</span>
                      <span className="text-[10px] text-blue-500 font-medium uppercase">{cita.servicio?.especialidad} — {cita.vehiculo?.tipo}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4 text-center">
                    <div className="flex justify-center gap-2">
                      ...
                      <button 
                        ...
                        disabled={!puedeAccionar}
                        className={`w-8 h-8 rounded-full border flex items-center justify-center transition-colors ${puedeAccionar ? 'text-red-400 border-red-100 hover:bg-red-50' : 'text-gray-200 border-gray-50 cursor-not-allowed'}`}
                      >✕</button>
                    </div>
                  </td>
                </tr>
          ...
          {loading && <div className="py-10 text-center text-gray-400">Sincronizando motores...</div>}
        </div>
      </div>

      {/* ─── MODAL ASIGNAR EQUIPO (DENTRO DEL COMPONENTE) ─── */}
      {modalAsignar && (
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setModalAsignar(null)} className="text-gray-400">✕</button>
              </div>
              <p className="text-gray-500 text-sm mb-4">Seleccione los técnicos que realizarán este trabajo:</p>
              <div className="space-y-3 max-h-80 overflow-y-auto mb-6">
                {tecnicosTaller.map(tec => (
                  <div 
                    key={tec.id} onClick={() => setTecnicoSeleccionado(tec.id)}
                    className={`p-4 rounded-2xl border-2 cursor-pointer transition-all flex items-center justify-between ${tecnicoSeleccionado === tec.id ? 'border-blue-500 bg-blue-50/50' : 'border-gray-100'}`}
                  >
                    ...
                    {tecnicoSeleccionado === tec.id && <span className="bg-blue-600 text-white text-[9px] font-black px-2 py-1 rounded-md">ASIGNADO</span>}
                  </div>
                ))}
              </div>
              <button onClick={confirmarYRecepcionar} className="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-blue-700 transition-all">
